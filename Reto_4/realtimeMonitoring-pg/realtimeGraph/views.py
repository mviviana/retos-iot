...
def get_map_json(request, **kwargs):
    ...
    if measureParam != None:
        selectedMeasure = Measurement.objects.a(name=measureParam)[0]
    elif measurements.count() > 0:
        selectedMeasure = measurements[0]
...
def get_data(request, **kwargs):
    ...
    city_name = kwargs.get("city", None)
    ...
    for measurement in measurements:
        for location in locations:
            stations = Station.objects.filter(location=location)
            locationData = Data.objects.filter(station__in=stations, measurement__name=measurement.name)
            if locationData.count() <= 0:
                continue
            minVal = locationData.aggregate(
                Min('value'))['value__min']
